    ...
    $.fn.blurjs = function(options) {
        ...
        if(cachedData) {
            ...
            tempImg.onload = function() {
                ...
                //With Worker
                var $imageWorker = new Worker('/bundles/twelvecouserauth/js/Workers/worker_blur.js');
                $imageWorker.addEventListener('message', function(e){
                    var data = e.data;
                    if (data.cmd === 'get-blur') {
                        ctx.putImageData(data.canvas[0], data.canvas[1], data.canvas[2]);
                        if(options.overlay) {
                            ctx.beginPath();
                ...
                var postMessageData = {
                    'cmd':               'get-blurred-image',
                    'canvas':            imageData,
                    'canvas_width':      canvas.width,
                    'canvas_height':     canvas.height,
                    'options_radius':    options.radius,
                    'iterations':        2
                };
                $imageWorker.postMessage(postMessageData);
            };
